...
LBPH_CONFIDENCE_THRESHOLD = float(os.environ.get("LBPH_CONFIDENCE_THRESHOLD", "52.0"))
...
def _extract_face_crop(img_gray):
    ...
    faces = face_cascade.detectMultiScale(
        img_gray, scaleFactor=1.1, minNeighbors=5, minSize=(100, 100)
    )
    if len(faces) == 0:
        return None
    if len(faces) > 1:
...
def register_person(req: RegisterRequest):
    ...
    if len(images_gray) < 5:
        raise HTTPException(
            status_code=400,
            detail=f"Could only detect a face in {len(images_gray)} image(s). Need at least 5.",
        )
